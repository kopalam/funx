...
namespace App\Services\WxSdk;
use App\Models\funx\TabCauths;
...
require_once base_path() .'/WxSdk/ErrorCode.php';
require_once base_path() .'/WxSdk/WXBizDataCrypt.php';
require_once base_path() .'/WxSdk/Prpcrypt.php';
require_once base_path() .'/WxSdk/PKCS7Encoder.php';
class WXLoginHelper {
    ...
    function findAppInfo($cauthIden)
    {
        $cauth  =   TabCauths::where(['cauth_iden',$cauthIden]);

            if(!$cauth)
                return ['code'=>10012,'message'=>'经查询无此appId'];
                // return $cauth;          


            if($cauth['status'] == 1)
                 return ['code'=>10011,'message'=>'当前小程序appId被禁用，请联系管理员'];
                
            
           return $cauth;
    }

    public function checkLogin($code, $rawData, $signature, $encryptedData, $iv,$cauthIden) {
        /**
         ...
         */
         ...
         $cauth     =   $this->findAppInfo($cauthIden);

         if(isset($cauth['code']))
             // throw new Exception($cauth['message'],$cauth['code']);
            return ['status'=>1,'message'=>$cauth['message'],'code'=>$cauth['code']];

        $params = [
            'appid' => $cauth['appid'],
            'secret' => $cauth['appsecret'],
            'js_code' => $code,
         ...
         * （使用官方提供的方法即可）
         */
        $pc = new WXBizDataCrypt($cauth['appid'], $sessionKey);
